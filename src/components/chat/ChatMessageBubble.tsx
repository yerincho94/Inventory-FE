...
export const ChatMessageBubble = ({
  ...
  const visibleSuggestions = parsedContent.suggestions.filter((suggestion) => !shouldHideSuggestion?.(suggestion));
                ...
                <section key={`${message.messageId}-section-${index}`} className="space-y-2">
                  ...
                  <div className="space-y-1.5">
                    {bodyLines.map((line, lineIndex) => {
                      ...
                      if (isBullet) {
                        return (
                          <div key={`${message.messageId}-${index}-${lineIndex}`} className="flex gap-2 text-sm leading-relaxed text-slate-700">
                            <span className="mt-1 text-sky-500">•</span>
                            <span className="whitespace-pre-wrap">{normalized}</span>
                          </div>
                        );
                      }
                      return <p key={`${message.messageId}-${index}-${lineIndex}`} className="whitespace-pre-wrap text-sm leading-relaxed text-slate-700">{trimmed}</p>;
                    })}
                  </div>
                </section>
              );
            }
            return <p key={`${message.messageId}-plain-${index}`} className="whitespace-pre-wrap text-sm leading-relaxed text-slate-700">{section}</p>;
          })}
        </div>
      );
    }
    return <div className={`${isUser ? 'text-gray-800' : 'text-slate-700'} whitespace-pre-wrap text-sm leading-relaxed`}>{content}</div>;
  };
      ...
      <div className={`flex max-w-[90%] sm:max-w-[85%] lg:max-w-[80%] gap-2 sm:gap-3 ${isUser ? 'flex-row-reverse' : 'flex-row'}`}>
        {!isUser && (
          <div className="flex h-8 w-8 sm:h-10 sm:w-10 flex-shrink-0 items-center justify-center overflow-hidden rounded-full border border-gray-100 bg-gray-50 shadow-sm">
            <img src="/images/chatbot.png" alt="수셰프" className="h-full w-full object-contain sm:object-cover" />
          </div>
        )}

        <div className="flex flex-col gap-2">
          <div className={`rounded-3xl px-3 py-2.5 sm:px-4 sm:py-3 shadow-[0_2px_8px_rgba(0,0,0,0.05)] ${isUser ? 'rounded-tr-sm border-2 border-sky-100 bg-white text-gray-800' : 'rounded-tl-sm border border-gray-100 bg-white text-gray-700'} ${isFailed ? 'border-red-200 bg-red-50/50' : ''} ${isInterrupted ? 'border-amber-200 bg-amber-50/60' : ''}`}>
            {(isFailed || isInterrupted) && (
              <div className={`mb-2 text-sm font-medium ${isFailed ? 'text-red-600' : 'text-amber-700'}`}>
                {message.errorMessage || (isInterrupted ? '답변 생성이 중단되었습니다' : '오류가 발생했습니다')}
              </div>
            )}
          ...
          {isFailed && onRetry && (
            <button type="button" onClick={() => onRetry(message)} className="flex items-center gap-2 rounded-lg px-2 sm:px-3 py-1.5 text-xs sm:text-sm text-red-600 hover:bg-red-50 hover:text-red-700">
